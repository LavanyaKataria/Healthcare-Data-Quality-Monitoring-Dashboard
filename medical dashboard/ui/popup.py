...
# #     if "Date"            in f:
# #         try:
# #             ddf = ddf[ddf["Verified Date"] == pd.Timestamp(f["Date"])]
# #         except Exception:
# #             pass
# #     if "Status" in f:
# #         if f["Status"] == "Issues":
# #             ddf = ddf[ddf["Has Issue"]]
# #         elif f["Status"] == "Pass":
# #             ddf = ddf[~ddf["Has Issue"]]
# #     if "kpi" in f:
# #         if f["kpi"] == "issues":
...
# #     # Render a native container styled as an enterprise card
# #     with st.container(border=True):
# #         col_title, col_close = st.columns([9, 3])
# #         with col_title:
# #             st.markdown(f"#### 🔍 Drill-Down: {title}")
# #         with col_close:
# #             if st.button("✕ Close Drill-Down", key="close_popup_btn", use_container_width=True, type="primary"):
# #                 close_popup()
...
# #         with c2:
# #             st.metric("Checks with Issues", f"{issue_rows:,}", delta=f"{issue_rows} issues" if issue_rows else None, delta_color="inverse")
# #         with c3:
...
# #         # Rename columns to upload originals
